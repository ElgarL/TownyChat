...
public class Chat extends JavaPlugin
{
	...
	public void onEnable()
	{
		...
		if ((towny == null)
				|| (getServer().getScheduler().scheduleSyncDelayedTask(this,
						new onLoadedTask(this), 1) == -1) || (!load()))
		{
			/*
	...
	private void checkPlugins()
	{
		...
		if (test != null)
		{
			try
			{
				if (Double.valueOf(test.getDescription().getVersion()) >= 3.1)
				{
					...
					getLogger()
							.warning(
									"TownyChat requires CraftIRC version 3.1 or higher to relay chat.");
			}
			catch (NumberFormatException e)
			{
				getLogger().warning(
						"Non number format found for craftIRC version string!");
			}
		}
		...
		if (irc != null)
		{
			...
			if (test != null)
			{
				...
				getLogger()
						.info("[TownyChat] Found and attempting to relay Heroic Death messages to craftIRC.");
			}
		}
	...
	public void unregisterPermissions()
	{
		...
		for (String perm : getChannelsHandler().getAllPermissions())
		{
			pm.removePermission(new Permission(perm,
					new HashMap<String, Boolean>()));
		}
	}
