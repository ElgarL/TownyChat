...
public class TownyChatPlayerListener implements Listener
{
	...
	public void onPlayerCommandPreprocess(PlayerCommandPreprocessEvent event)
	{
		...
		Channel channel = plugin.getChannelsHandler().getChannel(player,
				command);
		if (channel != null)
		{
			...
			if (message.isEmpty())
			{
				if (plugin.getTowny().hasPlayerMode(player, channel.getName()))
					...
					plugin.getTowny().setPlayerMode(player, new String[]
					{
						channel.getName()
					}, true);
	...
	public void onPlayerChat(AsyncPlayerChatEvent event)
	{
		...
		if (!isMuted(player))
		{
			...
			{
				Channel channel = plugin.getChannelsHandler().getChannel(
						player, directedChat.get(player));
			 ...
			 */
			for (Channel channel : plugin.getChannelsHandler().getAllChannels()
					.values())
			{
				if (plugin.getTowny().hasPlayerMode(player, channel.getName()))
				{
					/*
					 * Channel Chat mode set Process the chat
					 */
					channel.chatProcess(event);
					return;
				}
		...
		/*
		 * We found no channels available so modify the chat (if enabled) and
		 * exit.
		 */
		if (ChatSettings.isModify_chat())
		{
			try
			{
				event.setFormat(ChatSettings.getRelevantFormatGroup(player)
						.getGLOBAL().replace("{channelTag}", "")
						.replace("{msgcolour}", ""));
				Resident resident = TownyUniverse.getDataSource().getResident(
						player.getName());
			...
			try
			{
				if (plugin.getTowny().getEssentials().getUser(player).isMuted())
				{
					TownyMessaging.sendErrorMsg(player,
							"Unable to talk...You are currently muted!");
					return true;
				}
			}
	...
	/**
	 * Test if this player is spamming chat. One message every 2 seconds limit
	 * 
	 * @param player
	 * @return
	 */
	private boolean isSpam(Player player)
	{
		...
		{
			TownyMessaging.sendErrorMsg(player,
					"Unable to talk...You are spamming!");
			return true;
		}
		return false;
	}
